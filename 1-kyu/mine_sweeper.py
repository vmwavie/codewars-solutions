...
class Minefield:
        ...
        while True:
            if (self.n - self.bombs_found() <= 0 and len(self.question_marks()) == 0) or q.empty():
                ...
                if maybe_stuck == 0:
                    ...
                    to_queue = set()
                    for qm in qms:
                        to_queue |= set(qm.nearby_numbers())
                    for x in to_queue:
                        q.put(x)
                    maybe_stuck = 1
                    continue
    ...
    def solve_stuck(self, test_bombs=0):
        ...
        qms = set()
        for qm in all_qms:
            numbers |= set(qm.nearby_numbers())
        for number in numbers:
            qms |= set(number.nearby_unknowns())
        ...
        for bomb_amount in bomb_range:
            ...
            possibilities += list(set([tuple(sorted(p)) for p in ps]))
        ...
        for possibility in possibilities:
            ...
            for number in numbers:
                ...
                bad = False
                if len(possibility) == bombs_remaining:
                    for qm in self.question_marks():
                ...
                if not bad:
                    possible_values.append(possibility)
                    if solution:
                        for bomb in possibility:
                            bomb.value = '?'
        ...
        if solution and unique_solution:
